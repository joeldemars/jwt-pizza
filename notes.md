...
| Create a store                                      | createStore.tsx    | [POST] /api/franchise/:franchiseId/store |`SELECT id, name FROM franchise WHERE name LIKE ? LIMIT ${limit + 1} OFFSET ${offset}`<br/>`SELECT id, name FROM store WHERE franchiseId=?`<br/>`INSERT INTO store (franchiseId, name) VALUES (?, ?)`|
